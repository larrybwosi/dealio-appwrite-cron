...
import { uploadBlobToSanity } from "./upload";
    ...
    .map(
      (item) => `
        <tr>
            ...
            <td style="${styles.td} ${styles.textRight}">${currency}${item.quantity}</td>
            ...
            <div style="${styles.section}">
             ...
             <div style="${styles.section} ${styles.textRight}">
                <table style="width: 50%; margin-left: auto; border-collapse: collapse;">
                     <tr><td style="padding: 2px;">Subtotal:</td> <td style="padding: 2px;">${currency}${saleData.totalAmount}</td></tr>
                     <tr style="${styles.totalRow} border-top: 1px solid #ccc;">
                         <td style="padding: 4px 2px;">Total:</td>
                         <td style="padding: 4px 2px;">${currency}${saleData.finalAmount}</td>
                     </tr>
                     <tr style="border-top: 1px solid #eee;">
                         <td style="padding: 2px;">Paid (${saleData.paymentMethod}):</td>
                         <td style="padding: 2px;">${currency}${saleData.finalAmount}</td> {/* Assuming paid amount matches final */}
                     </tr>
                 </table>
             </div>
...
export async function generateAndSaveReceiptPdf(
  ...
  try {
    ...
    const receiptUrl = await uploadBlobToSanity(
